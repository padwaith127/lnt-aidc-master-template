...
## Purpose: To define the electrical and thermal interface requirements for 60kW to 120kW+ AI racks, focusing on {{ cookiecutter.ai_silicon_vendor }} infrastructure.
## Revision: 1.0
## Date: 24-May-2024
## Version: 1.0
## Tags: #AIRack #{{ cookiecutter.ai_silicon_vendor }} #HighDensity #PowerShelf #ORV3 #Busbar #{{ cookiecutter.project_code }}
## Related Files: [[Rack_Level_Distribution.md]], [[CDU_and_Pump_Electrical.md]], [[GPU_Infrastructure_Requirements.md]], [[Fault_Level_Calculations.md]]
## Standards Covered: OCP Open Rack V3 (ORV3), ASHRAE TC 9.9, IEC 62368-1, IEEE 1584.1

---

## 1. Overview
In the {{ cookiecutter.project_name }} {{ cookiecutter.city }} {{ cookiecutter.it_capacity_mw }} MW facility, the "AI Rack" is no longer just a piece of furniture; it is a high-performance power conversion engine. At {{ cookiecutter.rack_density_kw }}kW per rack ({{ cookiecutter.ai_silicon_vendor }}), the density is 10x to 20x higher than traditional enterprise racks. This requires a shift from AC-to-server distribution to a **Centralized DC Power Shelf** with a vertical **DC Busbar**.

## 2. The AI Power Shelf Architecture (ORV3 Standard)
Modern AI racks (OCP standard) utilize a centralized power shelf to maximize efficiency and serviceability.
*   **Input:** Dual 415V/480V 3-Phase AC feeds (Source A & B).
...
### 2.1 Why 48V DC?
At {{ cookiecutter.rack_density_kw }}kW, 12V DC distribution would require ~10,000 Amperes per rack, leading to massive $I^2R$ losses and unmanageable copper thickness. 48V DC reduces the current to ~2,500A, making distribution via a 3-inch busbar feasible.
...
*   **Total Heat ($Q_{total}$):** ${{ cookiecutter.rack_density_kw }} \text{ kW} = {{ (cookiecutter.rack_density_kw | float * 3412.14) | round(0) }} \text{ BTU/hr}$.
*   **Liquid vs. Air Split:** In a Direct-to-Chip (D2C) system:
    *   **Liquid (Cold Plate):** ~80% is removed by the CDU.
    *   **Air (Fans):** ~20% is rejected into the room.
*   **L&T Strategy:** The HVAC system in {{ cookiecutter.city }} must still provide ~20% of air cooling *per AI rack* even if liquid cooling is primary.

### 3.2 DC Busbar Sizing (Rear of Rack)
*   **Current ($I$):** ${{ (cookiecutter.rack_density_kw | float * 1000) | round(0) }} \text{ W} / 48 \text{ V} = {{ (cookiecutter.rack_density_kw | float * 1000 / 48) | round(0) }} \text{ Amperes}$.
...
3.  **Selective Coordination:** The branch DC fuses must trip faster than the Main Rectifier Protection to prevent a single GPU short from taking down the entire {{ cookiecutter.rack_density_kw }}kW rack.

## 5. Thermal-Electrical Integration (The "Cold Plate")
...
| :--- | :--- | :--- |
| **Overhead Busway** | 250A or 400A Tap-offs | To feed the {{ cookiecutter.rack_density_kw }}kW Power Shelf without floor clutter. |
| **Weight Loading** | 2,500 kg+ per rack | Fluid-filled racks + Copper busbars + GPUs are extremely heavy. |
| **EMC Shielding** | Class A / Class B | High-speed interconnect signals require isolation from VFD noise. |
| **Fiber Management** | Zero-U side channels | Massive amounts of networking fiber cannot block airflow. |
...
3.  **Flow-Power Interlock:** Simulate a CDU pump failure; verify the software/hardware initiates a "Graceful Shutdown" before the GPU temp reaches $85^\circ \text{C}$.

## 8. Failure Modes
*   **Busbar Contamination:** Dust/Humid air in {{ cookiecutter.city }} causing tracking on the 48V DC backplane. *Mitigation:* Use IP2X rated busbar shrouds.
...
- [ ] Is the DC busbar rated for the full continuous current?
- [ ] Are the AC input connectors (IEC 60309) specified with locking mechanisms?
...
* **Vendor Specs:** "{{ cookiecutter.ai_silicon_vendor }} System Architecture White Paper."
...
### Revision History
* 1.0: Initial AI Rack Electrical-Thermal Strategy for {{ cookiecutter.project_name }} {{ cookiecutter.city }}.

---
**Next File Recommendation:** `GPU_Infrastructure_Requirements.md` (Deep dive into the specific power profiles and infrastructure needs of AI clusters).
