...
## Purpose: To define the methodology for quantifying, estimating, and specifying electrical components for a {{ cookiecutter.it_capacity_mw }} MW AI-ready data centre.
## Revision: 1.0
## Date: 24-May-2024
## Version: 1.0
## Tags: #BOQ #Estimation #Procurement #CostEngineering #{{ cookiecutter.project_code }} #QuantitySurvey
...
## 1. Overview
The Bill of Quantities (BOQ) is the commercial heart of the {{ cookiecutter.project_name }} {{ cookiecutter.city }} project. It translates the 3D BIM model and 2D drawings into a line-item list of materials and labor. For an AI-ready facility, the BOQ must be exceptionally precise because specialized components (like {{ cookiecutter.rack_density_kw }}kW-ready busway or Li-ion BESS) have long lead times and high capital costs. Mistakes in quantification here directly lead to project delays or budget overruns.
...
| :--- | :--- | :--- |
| **Schedule A** | High Voltage (HT) System | {{ cookiecutter.utility_voltage_kv }}kV GIS, Transformers, HT Cabling, Outdoor Gantry. |
...
1.  **High-Density Track Busway:** Instead of standard 250A busway, the BOQ will reflect **800A to 1200A** track busway with a high density of **250A/400A Tap-off units**.
...
**Scenario:** A row of 10 {{ cookiecutter.ai_silicon_vendor }} racks, each {{ cookiecutter.rack_density_kw }}kW.
*   **Required Capacity:** 250A/400A per rack at 415V/480V.
...
*   **BOQ Line Items:**
    ...
    3.  *Tap-off Units:* 250A/400A, 3P, with Energy Metering & LSI Breaker - **10 Nos**.
...
*   **Lead Times:** Explicitly state the required "On-Site Date." For transformers in {{ cookiecutter.city }}, this may be **24-32 weeks**.
...
The BOQ should follow the construction logic of the {{ cookiecutter.city }} site:
...
### Revision History
* 1.0: Initial BOQ Methodology for {{ cookiecutter.project_name }} {{ cookiecutter.city }} project.

---
**Next File Recommendation:** `Master_Formula_Reference.md` (A centralized "Cheat Sheet" of every formula used in this repository for quick engineering checks).
