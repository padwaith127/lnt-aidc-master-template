...
## Purpose: To define the operational framework, procedures (SOP/EOP/MOP), and human-factor management for the electrical infrastructure of a {{ cookiecutter.it_capacity_mw }} MW AI-ready facility.
## Revision: 1.0
## Date: 24-May-2024
## Version: 1.0
## Tags: #Operations #SOP #EOP #MOP #DataCentreOps #{{ cookiecutter.project_code }} #UptimeStandard
## Related Files: [[Level_1_to_Level_5_Testing.md]], [[Preventive_and_Predictive.md]], [[Root_Cause_Methodology.md]], [[Supervisory_Control.md]]
## Standards Covered: Uptime Institute Operational Sustainability, ISO 27001, NFPA 70E, IEEE 3007.3

---

## 1. Overview
In a {{ cookiecutter.it_capacity_mw }} MW AI-ready facility, the "Design" and "Construction" phases represent only a fraction of the facility's life. The **Operations** phase is where the 99.999% availability is won or lost. For the {{ cookiecutter.city }} project, we must transition from an engineering mindset to an operational one, where **Human Error** (the cause of >70% of DC outages) is mitigated through rigorous documentation: SOPs, EOPs, and MOPs.
...
Routine operations ensure the "Health" of the {{ cookiecutter.it_capacity_mw }} MW site is visible.
...
2.  **Logbook Management:** Hourly recording of kWh, PUE, and Phase Balance at the {{ cookiecutter.utility_voltage_kv }}kV and LT Incomer levels.
3.  **Fuel Management:** Weekly testing of diesel quality (for {{ cookiecutter.city }} humidity) and fuel level verification.
...
*   **Sample MOP Workflow (Transferring a UPS to Maintenance Bypass):**
...
When a {{ cookiecutter.it_capacity_mw }} MW AI cluster experiences a power disturbance, the EOP must be instinctual.
1.  **Total Utility Outage (EOP-01):** Verify Auto-Start of all DG sets. If one fails to start, initiate manual sync.
...
1.  **{{ cookiecutter.ai_silicon_vendor }} "Training Day" Coordination:** When an AI model begins training, the IT load can jump from 10% to 100% in seconds. Operations must coordinate with the client to ensure the **Cooling Plant** is in "Pre-compute" mode (ramped up) before the power spike.
2.  **Transient Monitoring:** Operators must use the EPMS to review transient events after every AI "Training Run" to identify potential weak points in the distribution (e.g., loose busbar joints).
...
*   **Lock-Out Tag-Out (LOTO):** Mandatory for every maintenance task in the {{ cookiecutter.city }} facility. Site engineers must be trained "Authorized Persons."
*   **Arc Flash Boundaries:** Operators must never enter the "Prohibited Space" of {{ cookiecutter.utility_voltage_kv }}kV switchgear without the correct Category 4 PPE, as defined in the **Arc Flash Study**.
...
- [ ] Is the PUE within the target range (<{{ cookiecutter.target_pue }})?
...
### Revision History
* 1.0: Initial Operational Strategy for {{ cookiecutter.project_name }} {{ cookiecutter.city }} project.

---
**Next File Recommendation:** `Preventive_and_Predictive.md` (Focusing on the technical maintenance of HT/LT gear, Transformers, and DGs).
